...
Gitignored, and therefore absent on a fresh clone: `.claude` (hence the `setup.sh` symlink step), `data` (every task's dataset — fetched via `fetch_data.py` where `task.json` declares a source, otherwise supplied separately), `test/*/runs`, `test/logs`, `test/notebooks`, `test/results`, `.vscode`, and all `*.ipynb`. These are local run history/scratch/inputs, not source.
