...
`test/test.py <task-dir-name> [--skills] [--timeout MINUTES] [--n COUNT] [--provider {llama-cpp,anthropic,openai}] [--model MODEL] [--run-id ID] [--prepare-only]` — run from inside `test/` — is the whole harness. It is deliberately a single Python script, not a package; keep changes in that style. `test/run_all.py` is a thin driver on top of it (see below).
...
**`--provider`** selects which pi provider runs the agent: `llama-cpp` (default) for the host's local llama.cpp router — `LLAMA_BASE_URL` is passed into the container so pi finds it — or one of the **hosted** providers, `anthropic` and `openai`, both pi *built-ins* (already present in `pi-coding-agent`'s own dependencies — unlike `llama-cpp`, they need no separate package install in `docker-agent/Dockerfile`). `test.py`'s `HOSTED_PROVIDER_KEYS` maps each to its one credential env var (`ANTHROPIC_API_KEY` / `OPENAI_API_KEY`), which must be on the host — exported in the shell or, more usually, sitting in the gitignored root `.env` (see `test/env_file.py` below); `test.py` passes **only that provider's key** into the container as an env var (an `openai` run's container never sees the Anthropic key, or vice versa), never bakes it into the image, and dies before touching Docker if it's unset. Adding a third hosted provider means one entry in that dict, nothing else. The container isn't network-isolated (see `docker-agent/`'s isolation notes above), so it reaches `api.anthropic.com` / `api.openai.com` over normal egress with no other config. `--model` under a hosted provider is a literal API model id (`claude-opus-5`, `gpt-5.5`, …) with no router to resolve or validate against — required outright, same "no hardcoded default" reasoning as the `llama-cpp` case below. **Nothing validates the id against a list, and pi doesn't either** — it forwards whatever it's given straight to the provider's API, so a model newer than pi's bundled catalog runs unchanged: pi 0.83.0's catalog stops at `claude-opus-4-8`/`gpt-5.5`, yet the recorded `claude-opus-5` runs scored 1.0. A wrong id therefore fails at the API, inside the container, as a 4xx in `run.jsonl` — not up front. `eval_result.json`'s `model_server` field records the provider id (`anthropic`, `openai`) for these runs, so they stay distinguishable from `llama.cpp` runs when the recorded results are compared. `run_all.py` forwards both `--provider` and `--model` to every `test.py` it spawns (omitting each flag when not given, so the defaults stay `test.py`'s), which is what makes a full Anthropic sweep a single `./run_all.py --provider anthropic --model claude-opus-5` rather than a manual invocation per task. Neither flag is part of the `.run_all_state.json` checkpoint's identity, so `--resume` continues the position in the sweep regardless of which model produced the earlier runs — pass the same flags on resume. Because `run_all.py` suppresses `test.py`'s output, it checks `ANTHROPIC_API_KEY` itself before starting (`test.py`'s own `die()` would be invisible, and every run would fail silently).
...
**`test/run_all.py`** runs the full matrix — every task directory containing a `task.json`, 5 iterations with `--skills` and 5 without, 60-minute timeout each (constants at the top of the file). It suppresses `test.py`'s stdout because `test.py` interleaves the model's raw output with its own progress and has no flag to separate them; progress is reported instead from each run's `eval_result.json` as it lands, and the model's full output is still on disk in `run.txt`/`run.jsonl`. It checkpoints to `test/.run_all_state.json` (gitignored) after every run that produced an `eval_result.json` — a run that died without grading isn't checkpointed, so it's retried. What it records is the last run of an unbroken *completed prefix* of the plan, not the last run to finish: under `--parallel`, runs land out of order, and recording run 5 while run 4 is still in flight would make a later `--resume` skip run 4 entirely. **`--resume`** picks the sweep up at the run after that checkpoint; it falls back to starting from the beginning (with a logged reason) if the state file is missing or names a run no longer in the plan (e.g. a task directory was added or removed).

**Every run is given an explicit `--run-id`** of the form `<timestamp>_<s|n><iteration>` (e.g. `20260813_213000_s3`), which becomes both the run directory name and part of the container name. `test.py`'s own default id is only the start timestamp to the second — unique within one process, but *not* across concurrent `test.py` processes, which would otherwise put two runs of the same task in one directory and have them overwrite each other's outputs. Each step appears exactly once in the plan, so the id is collision-free by construction; `run_all.py` then reads the run back by that id instead of diffing `runs/` before and after (which can't attribute a directory to a run when several are in flight). Run-directory names are opaque labels to everything downstream — `analysis/`'s `load_runs()` reads `eval_result.json` and never parses the name — so the format is free to change.

**`--parallel N`** runs N containers at once (default 1 = the previous serial behaviour, byte-identical in what it does). Before fanning out it runs `test.py <task> --prepare-only` once per task, serially: that flag fetches `data/` and builds `pytcr-test-<task>`, then exits without starting an agent (so it needs neither a model nor an API key). Without that pass, N concurrent runs of one task would each do the same setup simultaneously — several `fetch_data.py` processes writing the same `data/` files, and competing `docker build -t` of the same tag. Two caveats worth stating to anyone raising N: each container is capped at `MEMORY_GB` (default 8) so N of them want N×that, and N only buys throughput if the model server can serve N agents at once — a local llama.cpp router with `parallel = 1` in its preset serializes them, which is why `run_all.py` logs a warning when `--parallel > 1` meets the local provider. It's the hosted providers (`anthropic`, `openai`) that actually parallelize. A Ctrl-C cancels queued runs and lets the in-flight ones stop the way they always do (`docker stop`, grade what exists), then exits 130.

**`test/env_file.py`** is the harness's only shared module: a ~40-line KEY=VALUE parser that loads the gitignored **root `.env`** into `os.environ`, imported at the top of both `test.py` and `run_all.py` (in `test.py` it must stay above the module-level `CONTAINER_LLAMA_URL`/`HOST_LLAMA_URL` constants, which read the environment at import time). Anything already exported in the shell wins over the file, so `MODEL=... ./test.py ...` still overrides it. It's hand-rolled rather than python-dotenv on purpose — these scripts run under whatever Python the host has, with nothing to install. In practice `.env` is where `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` live, which is why a Claude sweep needs no key on the command line; `MODEL`, `MEMORY_GB`, `LLAMA_BASE_URL` and `HOST_LLAMA_URL` work there too. Both scripts log which *names* they picked up (never values). **`docker-agent/chat.sh` reads the same `.env`** (`ANTHROPIC_API_KEY`, `MODEL`, `LLAMA_PORT`) with the same shell-wins precedence, reimplemented in bash in its `load_env_file` — deliberately parsed rather than `source`d, since `.env` is data and sourcing it would execute it and clobber exported variables. Two implementations of the same tiny format: change one and change the other.
...
./docker-agent/chat.sh [model-id]          # defaults to MODEL (env or .env), else whichever model is loaded
./docker-agent/chat.sh --provider anthropic claude-opus-5   # hosted Claude instead (key from the shell or .env)
./docker-agent/chat.sh --provider openai gpt-5.5            # hosted OpenAI instead (OPENAI_API_KEY)
...
./test.py 01-data-loading --provider anthropic --model claude-opus-5   # compare against Claude (key from the root .env, or export it)
./test.py 01-data-loading --provider openai --model gpt-5.5             # ... or against OpenAI (OPENAI_API_KEY)
...
./run_all.py --parallel 4                         # 4 containers at once (prepares data/images serially first)
./run_all.py --provider anthropic --model claude-opus-5   # whole sweep against Claude
./run_all.py --provider openai --model gpt-5.5            # whole sweep against OpenAI
# ANTHROPIC_API_KEY / OPENAI_API_KEY / MODEL / MEMORY_GB / LLAMA_BASE_URL are read from the root .env when not exported
